...
def capture_face(profile_name):
    ...
    while True:
        ...
        if key == ord("s"):
            ...
            # Save the profile image for manual inspection
...
def authenticate():
    ...
    while True:
        ...
        if len(faces) > 0:
            ...
            # Compare with profiles
            for name, saved_face in profiles.items():
                # Normalize face images for better comparison
                face_img_norm = face_img / 255.0
                saved_face_norm = saved_face / 255.0

                # Compute Mean Squared Error (MSE)
                mse = np.mean((face_img_norm - saved_face_norm) ** 2)

                if mse < 0.02:  # Adjust threshold for more tolerance
                    print(f"Authenticated as {name}")
                    cam.release()
                    cv2.destroyAllWindows()
                    return
